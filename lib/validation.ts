...
export function validateBooking(travelers: number, date: string, locale: Locale = 'en'): string | null {
  if (!Number.isInteger(travelers) || travelers < 1 || travelers > MAX_TRAVELERS)
    return locale === 'ar' ? `عدد المسافرين بين 1 و ${MAX_TRAVELERS}.` : `Travelers must be between 1 and ${MAX_TRAVELERS}.`
