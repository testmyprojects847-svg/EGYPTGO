...
function AdminDashboard() {
  const { dir } = useLanguage(); const { format } = useCurrency(); const searchParams = useSearchParams(); const [tab, setTab] = useState(searchParams.get('tab') ?? 'tours'); const [sidebarOpen, setSidebarOpen] = useState(false); const stats = bookingService.stats()
  const active = tab === 'dashboard' ? 'tours' : tab
  const content = tab === 'tours' ? <><div className="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(330px,0.75fr)]"><div className="min-w-0 rounded-2xl border border-border bg-card p-5 shadow-sm"><TourTable /></div><div className="min-w-0 rounded-2xl border border-border bg-card p-5 shadow-sm"><GuideWorkspace /></div></div><RecentBookings /></> : tab === 'guides' ? <div className="rounded-2xl border border-border bg-card p-5 shadow-sm"><GuideWorkspace /></div> : tab === 'bookings' ? <div className="rounded-2xl border border-border bg-card p-5 shadow-sm"><BookingTable /></div> : tab === 'destinations' ? <DestinationForm /> : tab === 'hotels' ? <HotelForm /> : tab === 'users' ? <UserTable /> : tab === 'reviews' ? <ReviewModeration /> : tab === 'settings' ? <SettingsWorkspace /> : <PaymentTable />
  return <div dir={dir} className="min-h-screen bg-muted/30 md:flex"><AdminSidebar active={active} onChange={setTab} open={sidebarOpen} onClose={() => setSidebarOpen(false)} /><div className="min-w-0 flex-1"><AdminHeader onMenu={() => setSidebarOpen(true)} /><main className="mx-auto flex max-w-[1600px] flex-col gap-6 px-4 py-6 lg:px-8 lg:py-8"><div className="flex flex-col gap-2"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-accent">EgyptGO operations</p><h2 className="text-2xl font-bold tracking-tight sm:text-3xl">Your travel marketplace, at a glance</h2><p className="max-w-2xl text-sm leading-6 text-muted-foreground">Manage experiences, local experts, and traveler demand from one focused workspace.</p></div><StatCards items={[{ label: 'Total bookings', value: String(stats.total), change: '+12.5%' }, { label: 'Total revenue', value: format(stats.revenue), change: '+8.2%' }, { label: 'Total tours', value: String(tourService.list().length), change: '+4.3%' }, { label: 'Total users', value: String(users.length), change: '+18.7%' }]} />{content}</main></div></div>
}
